##-----------------------------------------------------------------------------
# provider: aws
# name:     instance_deployment-linux
# version:  1.11.0
##-----------------------------------------------------------------------------
...
locals {
  ...
  purpose = {
    ...
    machine = (try(var.details.purpose_machine, "") != "" ? var.details.purpose_machine : can(var.details.purpose_abbr) ? replace(var.details.purpose_abbr, "/[^0-9A-Za-z]/", "") : lower(replace(var.details.purpose, "/[^0-9A-Za-z]/", "")))
  }
